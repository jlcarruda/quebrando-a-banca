...
class ChampMostEffectiveRoles(Miner):
    ...
    def __init__(self, rootpath=".."):
        self.df_champions = self.pd.read_csv(
            rootpath + '/datasets/championFull.csv')
    ...
    def __mapFunc(self, yearDataframe):
        roleCounts = {}
        # Grafico de comps mais relevantes
        sortedDf = yearDataframe.sort_values(
            by="quantidade_vitorias", ascending=False)
        # ====> contagem de papeis
    ...
    def __countRoles(self, comp, counter):
        ...
        champ1_role = self.df_champions[self.df_champions.champion_Name ==
                                        champ1]["tags"].values
        if len(champ1_role) > 0:
            keys_1 = eval(champ1_role[0])
            map(lambda key: self.__addKeys(key, counter), keys_1)

        champ2_role = self.df_champions[self.df_champions.champion_Name ==
                                        champ2]["tags"].values
